...
// Function to add a user with SHA-256 password hashing
int add_user(const char *username, const char *password) {
    char hashed_pass[65];  // SHA-256 hex output
    sha256(password, hashed_pass);

    FILE *shadow_file = fopen(SHADOW_FILE, "a");
    if (!shadow_file) {
